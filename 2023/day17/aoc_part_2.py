...
def get_number_part(input_file_name):
    ...
    heatloss = {}
    current_path = []
    heapq.heappush(current_path, (0, 1, 'e', 1, int(lines[0][1]), [(0, 1)]))
    heapq.heappush(current_path, (1, 0, 's', 1, int(lines[1][0]), [(1, 0)]))
    rt, ct = (len(lines)-1, len(lines[-1])-1)
    while len(current_path) > 0:
        r, c, d, steps, hl, path = heapq.heappop(current_path)
        #print(r, c, d, steps, hl)
        key = (r, c, d, steps)
        ...
        if r == rt and c == ct and steps >= 4:
            result = min(hl, result)
            #print(hl, result, path)
            continue
        moves = {
        ...
        for m in moves[d]:
            ...
            if rn >= 0 and rn < len(lines) and cn >= 0 and cn < len(lines[rn]):
                if dn == d:
                    if steps < 10:
                        new_path = path.copy()
                        new_path.append((rn, cn))
                        heapq.heappush(current_path, (rn, cn, dn, steps+1, hl+int(lines[rn][cn]), new_path))
                elif steps >= 4:
                    new_path = path.copy()
                    new_path.append((rn, cn))
                    heapq.heappush(current_path, (rn, cn, dn, 1, hl + int(lines[rn][cn]), new_path))
